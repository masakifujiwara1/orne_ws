...
#		self.scan_sub = rospy.Subscriber("/scan", LaserScan, self.callback_scan)
		...
		with open(self.path + self.start_time + '/' +  'reward.csv', 'w') as f:
			...
			writer.writerow(['step', 'mode', 'loss', 'angle_error(rad)', 'distance(m)','count'])

		with open(self.path + self.start_time + '/' +  'dataset.csv', 'w') as f:
			writer = csv.writer(f, lineterminator='\n')
...
#            			target_action = self.joy_angular
			...
			if self.angle_error > 0.05:
				self.action, self.loss = self.dl.act_and_trains(imgobj, target_action)
    				line_dataset = [str(target_action)]
    				with open(self.path + self.start_time + '/' + 'dataset.csv', 'a') as f:
					writer = csv.writer(f, lineterminator='\n')
					writer.writerow(line_dataset)
				self.count += 1
			...
			print(" episode: " + str(self.episode) + ", loss: " + str(self.loss) + ", angle_error: " + str(self.angle_error) + ", distance: " + str(distance), ", count: " + str(self.count))
			self.episode += 1
			line = [str(self.episode), "training", str(self.loss), str(self.angle_error), str(distance), str(self.count)]
			...
			self.angle_error = abs(self.action - target_action)
			line = [str(self.episode), "test", "0", str(self.angle_error), str(distance)]
			with open(self.path + self.start_time + '/' + 'reward.csv', 'a') as f:
				writer = csv.writer(f, lineterminator='\n')
				writer.writerow(line)
			self.vel.linear.x = 0.2
