...
class cource_following_learning_node:
	def __init__(self):
		...
		with open(self.path + self.start_time + '/' +  'reward.csv', 'w') as f:
			...
			writer.writerow(['step', 'mode', 'loss', 'angle_error(rad)', 'distance(m)','dataset'])
...
#			return
		...
		if self.learning:
			...
			self.episode += 1
			line = [str(self.episode), "training", str(loss), str(angle_error), str(distance), str(target_action)]
